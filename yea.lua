...
ESPTab:CreateToggle({
    ...
    Callback = function(value)
        if value then
            -- Use the currently selected player from dropdown
            local currentSelection = espDropdown.CurrentOption
...
FilesTab:CreateButton({
    ...
    Callback = function()
        ...
        Rayfield:Notify({ 
            Title = "File Scan", 
            Content = "Game structure scanned! Check console for details.", 
            Duration = 3 
        })
        print("=== GAME FILES STRUCTURE ===")
        print(HttpService:JSONEncode(files))
    end,
})

FilesTab:CreateButton({
    Name = "View Workspace Objects",
    Callback = function()
        local workspaceObjects = {}
        for _, obj in pairs(workspace:GetChildren()) do
            table.insert(workspaceObjects, {
                Name = obj.Name,
                Class = obj.ClassName
            })
        end
        ...
        Rayfield:Notify({ 
            Title = "Workspace", 
            Content = "Found " .. #workspaceObjects .. " objects in workspace!", 
            Duration = 3 
        })
        print("=== WORKSPACE OBJECTS ===")
        for _, obj in pairs(workspaceObjects) do
            print(obj.Name .. " (" .. obj.Class .. ")")
        end
    end,
})
